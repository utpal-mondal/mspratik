...
const EditUserPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchUser = async () => {
      try {
        setLoading(true);
        const res = await settingsApi.getUserById(Number(id));
        if (res?.data) {
          const user = res.data;
          setFormData((prev) => ({
            ...prev,
            ...
            language: user.language === "en" ? "English" : "Dutch",
            portalNotifications: user.receive_notify || false,
            allowedIPs: user.login_allowed_ip || false,
          }));
          if (user.role_id) {
            setSelectedRole({ id: user.role_id, name: user.role_name || "" });
          }
        }
      } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await settingsApi.updateUser(Number(id), {
